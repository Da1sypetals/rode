/**
 * RoDe SDDMM PyTorch CUDA 扩展
 * 
 * 提供 PyTorch tensor 接口来调用 RoDe SDDMM CUDA kernel
 */
...
void RoDeSDDMM_n32(
    ...
    const int* __restrict__ row_indices_block,
    const int* __restrict__ row_indices_residue,
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, int, int> rode_preprocess(
    ...
    int nnz,                     // 非零元素数
    int seg_length,              // 段长度：k=32时为512，k=128时为32
...
// RoDe SDDMM 前向传播
// ============================================================================
torch::Tensor rode_sddmm_forward(
    // RoDe 元数据
    ...
    int n,                          // 列数
    // 稠密矩阵
    torch::Tensor lhs_matrix,       // 左矩阵 (m, k)
    torch::Tensor rhs_matrix,       // 右矩阵 (n, k)
    // 选项
    int k_version                   // k 版本：32 或 128
) {
    // 检查输入
    ...
    CHECK_FLOAT(values);
    CHECK_FLOAT(lhs_matrix);
    CHECK_FLOAT(rhs_matrix);
    
    int k = lhs_matrix.size(1);
    int nnz = values.size(0);
    
    TORCH_CHECK(k_version == 32 || k_version == 128, "k_version must be 32 or 128");
    TORCH_CHECK(k == k_version, "k dimension must match k_version");
    TORCH_CHECK(lhs_matrix.size(0) == m, "lhs_matrix rows must equal m");
    TORCH_CHECK(rhs_matrix.size(0) == n, "rhs_matrix rows must equal n");
    TORCH_CHECK(rhs_matrix.size(1) == k, "rhs_matrix cols must equal k");
    
    // 创建输出 tensor
    auto out = torch::zeros({nnz}, values.options());
    ...
    const float* d_lhs_matrix = lhs_matrix.data_ptr<float>();
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("sddmm_forward", &rode_sddmm_forward,
          "RoDe SDDMM forward (CUDA)",
