...
# 源文件列表
sources = [
    os.path.join(current_dir, "rode_spmm_cuda.cu"),
    os.path.join(current_dir, "RoDeSpmm.cu"),
]

# 头文件目录
include_dirs = [
    current_dir,
    Path(__file__).resolve().parent,  # 包含头文件的目录
]
...
setup(
    name="rode_spmm_cuda",
    version="1.0.0",
    description="RoDe SPMM CUDA Extension for PyTorch",
    author="RoDe Team",
    ext_modules=[
        CUDAExtension(
            name="rode_spmm_cuda",
